...
extern "C" void fluid_msleep(unsigned int msecs)
{
    std::this_thread::sleep_for(std::chrono::milliseconds(msecs));
}

extern "C" double fluid_utime()
{
...
extern "C" void delete_fluid_thread(fluid_thread_t *_thread)
{
...
extern "C" int fluid_thread_join(fluid_thread_t *thread)
{
    static_cast<std::thread *>(thread)->join();
    return FLUID_OK;
}

extern "C" void _fluid_mutex_init(fluid_mutex_t *mutex)
{
...
extern "C" void fluid_mutex_destroy(fluid_mutex_t mutex)
{
...
extern "C" void _fluid_mutex_lock(fluid_mutex_t *mutex)
{
    if (*mutex == nullptr)
    {
...
extern "C" void fluid_mutex_unlock(fluid_mutex_t mutex)
{
    static_cast<std::mutex *>(mutex)->unlock();
}

extern "C" void _fluid_rec_mutex_init(fluid_rec_mutex_t *mutex)
{
...
extern "C" void fluid_rec_mutex_destroy(fluid_rec_mutex_t mutex)
{
    delete static_cast<std::recursive_mutex *>(mutex);
}

extern "C" void fluid_rec_mutex_lock(fluid_rec_mutex_t mutex)
{
    ensure_lock_mutex(static_cast<std::recursive_mutex *>(mutex));
}

extern "C" void fluid_rec_mutex_unlock(fluid_rec_mutex_t mutex)
{
    static_cast<std::recursive_mutex *>(mutex)->unlock();
}

extern "C" void fluid_cond_mutex_lock(fluid_cond_mutex_t *mutex)
{
    ensure_lock_mutex(static_cast<std::mutex *>(mutex));
}

extern "C" void fluid_cond_mutex_unlock(fluid_cond_mutex_t *mutex)
{
    static_cast<std::mutex *>(mutex)->unlock();
}
...
extern "C" void delete_fluid_cond_mutex(fluid_cond_mutex_t *mutex)
{
    delete static_cast<std::mutex *>(mutex);
}

extern "C" void fluid_cond_signal(fluid_cond_t cond)
{
    static_cast<std::condition_variable *>(cond)->notify_one();
}

extern "C" void fluid_cond_broadcast(fluid_cond_t cond)
{
    static_cast<std::condition_variable *>(cond)->notify_all();
}

extern "C" void fluid_cond_wait(fluid_cond_t cond, fluid_cond_mutex_t *mutex)
{
...
extern "C" void delete_fluid_cond(fluid_cond_t cond)
{
    delete static_cast<std::condition_variable *>(cond);
}

extern "C" void _fluid_private_init(fluid_private_t *priv)
{
    *priv = priv;
}

extern "C" void fluid_private_free(fluid_private_t priv)
{
...
extern "C" void fluid_private_set(fluid_private_t priv, void *value)
{
...
extern "C" int fluid_stat(const char *_path, fluid_stat_buf_t *buffer)
{
    try
    {
