...
#include "fluid_sys.h"
#include <cstdlib> /* RAND_MAX (for type only) */

/* Minimal LCG rand replacement — avoids system rand() not exported on Android API 17 bionic */
static unsigned int _dither_seed = 12345u;
static inline int fluid_dither_rand(void) {
    _dither_seed = _dither_seed * 1103515245u + 12345u;
    return (int)(_dither_seed / 65536u) % 128;
}
...
extern "C" {
    ...
    void init_dither(void)
    {
        ...
        for(c = 0; c < DITHER_CHANNELS; c++)
        {
            ...
            {
                d = fluid_dither_rand() / 128.0f - 0.5f;
                rand_table[c][i] = d - dp;
                dp = d;
            }
